...
class ExhibitorAnalytics extends Component
{
    use WithPagination;
    ...
    protected function getBaseQuery()
    {
        $query = AnalyticsEvent::query()
            ->where(function ($q) {
                $q->where('trackable_type', Exhibitor::class)
                    ->where('trackable_id', $this->exhibitor->id)
                    ->orWhereIn('trackable_id', $this->exhibitor->projects->pluck('id'))
    ...
    public function render()
    {
        ...
        return view('livewire.exhibitor-analytics', [
            'events' => $events,
